...
static int (*const decoder_table[3])(struct mad_stream *,
                                     struct mad_frame *) = {
...
static int free_bitrate(struct mad_stream *stream,
  ...
  slots_per_frame =
      (header->layer == MAD_LAYER_III && (header->flags & MAD_FLAG_LSF_EXT))
          ? 72
          : 144;
  ...
  while (mad_stream_sync(stream) == 0) {
    ...
    if (decode_header(&peek_header, &peek_stream) == 0 &&
      ...
      if (header->layer == MAD_LAYER_I) {
        rate = (unsigned long)header->samplerate * (N - 4 * pad_slot + 4) / 48 /
               1000;
      } else {
        rate = (unsigned long)header->samplerate * (N - pad_slot + 1) /
