...
static struct {
  unsigned char slen1;
  unsigned char slen2;
} const sflen_table[16] = {{0, 0}, {0, 1}, {0, 2}, {0, 3}, {3, 0}, {1, 1},
...
static mad_fixed_t const cs[8] = {
    ...
    +MAD_F(0x0fff964c) /* +0.999899195 */,
    +MAD_F(0x0ffff8d3) /* +0.999993155 */
};
...
static mad_fixed_t const ca[8] = {
    ...
    -MAD_F(0x003a2847) /* -0.014198569 */,
    -MAD_F(0x000f27b4) /* -0.003699975 */
};
...
static mad_fixed_t const is_lsf_table[2][15] = {
    {
        ...
        MAD_F(0x016a09e6) /* 0.088388348 */, MAD_F(0x01306fe1) /* 0.074325445 */
    },
    {
        MAD_F(0x0b504f33) /* 0.707106781 */,
        MAD_F(0x08000000) /* 0.500000000 */,
        ...
        MAD_F(0x00200000) /* 0.007812500 */, MAD_F(0x0016a09e) /* 0.005524272 */
    }};
 ...
 */
static enum mad_error III_sideinfo(struct mad_bitptr *ptr, unsigned int nch,
...
static void III_exponents(struct channel const *channel,
  ...
  if (channel->block_type == 2) {
    ...
    while (l < 576) {
      exponents[sfbi + 0] = gain0 - (signed int)(channel->scalefac[sfbi + 0]
                                                 << scalefac_multiplier);
      exponents[sfbi + 1] = gain1 - (signed int)(channel->scalefac[sfbi + 1]
                                                 << scalefac_multiplier);
      exponents[sfbi + 2] = gain2 - (signed int)(channel->scalefac[sfbi + 2]
                                                 << scalefac_multiplier);
    ...
    if (channel->flags & preflag) {
      for (sfbi = 0; sfbi < 22; ++sfbi) {
        exponents[sfbi] =
            gain - (signed int)((channel->scalefac[sfbi] + pretab[sfbi])
                                << scalefac_multiplier);
      }
    } else {
      for (sfbi = 0; sfbi < 22; ++sfbi) {
        exponents[sfbi] =
            gain - (signed int)(channel->scalefac[sfbi] << scalefac_multiplier);
      }
    }
  }
}
  ...
  /* count1 */
  {
    ...
    while (cachesz + bits_left > 0 && xrptr <= &xr[572]) {
      ...
      if (!quad->final) {
        ...
        quad =
            &table[quad->ptr.offset + MASK(bitcache, cachesz, quad->ptr.bits)];
      }
      ...
      xrptr[0] = quad->value.v ? (MASK1BIT(bitcache, cachesz--) ? -requantized
                                                                : requantized)
                               : 0;

      /* w (0..1) */

      xrptr[1] = quad->value.w ? (MASK1BIT(bitcache, cachesz--) ? -requantized
                                                                : requantized)
                               : 0;
      ...
      xrptr[0] = quad->value.x ? (MASK1BIT(bitcache, cachesz--) ? -requantized
                                                                : requantized)
                               : 0;

      /* y (0..1) */

      xrptr[1] = quad->value.y ? (MASK1BIT(bitcache, cachesz--) ? -requantized
                                                                : requantized)
                               : 0;
 ...
 */
static void III_reorder(mad_fixed_t xr[576], struct channel const *channel,
 ...
 */
static inline void imdct36(mad_fixed_t const X[18], mad_fixed_t x[36]) {
 ...
 */
static void III_imdct_l(mad_fixed_t const X[18], mad_fixed_t z[36],
...
static enum mad_error III_decode(struct mad_bitptr *ptr,
                                 struct mad_frame *frame, struct sideinfo *si,
                                 unsigned int nch) {
  ...
  for (gr = 0; gr < ngr; ++gr) {
    ...
    for (ch = 0; ch < nch; ++ch) {
      ...
      if (header->flags & MAD_FLAG_LSF_EXT) {
        part2_length =
            III_scalefactors_lsf(ptr, channel, ch == 0 ? 0 : &si->gr[1].ch[1],
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame) {
  ...
  error = III_sideinfo(&stream->ptr, nch, header->flags & MAD_FLAG_LSF_EXT, &si,
  ...
  if (frame_free >= next_md_begin) {
    ...
    memcpy(*stream->main_data + stream->md_len, stream->next_frame - frame_free,
           frame_free);
